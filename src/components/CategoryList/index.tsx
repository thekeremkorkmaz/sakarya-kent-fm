    ...
    <div>
      ...
      <div className='flex flex-wrap justify-start gap-5 px-4 mt-10'>
        {data.categories?.map((item: CateProps) => (
          <Link href={`/blog/${item.title}`} key={item.title} className={`flex gap-3 bg-${item.color}-300 items-center opacity-95 justify-center py-5 w-44 max-md:w-32 rounded-lg text-md font-medium `}>
