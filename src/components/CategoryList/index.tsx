    ...
    <div>
      ...
      <div className='flex flex-wrap justify-start gap-5 px-4 mt-10'>
        {data.data.map((item: CateProps) => (
          <Link href={`/blog/${item.name}`} key={item.name} className={`flex gap-3 bg-${item.color}  items-center  justify-center py-5 w-44 max-md:w-32 rounded-lg text-md font-medium `}>
